    ...
    func querySpaces() -> [Space] {
        ...
        for (dindex, display) in displays.enumerated() {
            ...
            for nsSpace:NSDictionary in displaySpaces {
                ...
                spaces.append(Space(id: spaceId, uuid: spaceUUID, visible: visible, active: active, display: dindex + 1, index: spaceIndex, yabaiIndex: totalSpaces, type: spaceType))
            }
        }
        return spaces
    }
    ...
    func queryDisplays() -> [Display] {
        ...
        if let uuids = rawUuids {
            for (i, displayUuid) in uuids.enumerated() {
                ...
                displays.append(Display(id: UInt64(did), uuid: displayUuid as String, index: i, size: bounds.size))
            }
        }
        return displays
    }
